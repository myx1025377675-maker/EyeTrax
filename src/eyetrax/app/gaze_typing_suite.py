...
def run_gaze_typing_suite():
    ...
    last_gaze_pos: Optional[tuple[int, int]] = None
    max_jump_px = int(min(screen_width, screen_height) * 0.18)
    ...
    with camera(camera_index) as cap, fullscreen(window_name):
        ...
        for frame in iter_frames(cap):
            ...
            if features is not None and not blink_detected:
                ...
                if last_gaze_pos is not None:
                    lx, ly = last_gaze_pos
                    dx, dy = x_pred - lx, y_pred - ly
                    dist = float(np.hypot(dx, dy))
                    if dist > max_jump_px:
                        ratio = max_jump_px / dist
                        x_pred = int(lx + dx * ratio)
                        y_pred = int(ly + dy * ratio)
                last_gaze_pos = (x_pred, y_pred)
            else:
                x_pred = y_pred = None
